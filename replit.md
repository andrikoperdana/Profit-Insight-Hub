...
**Email** (`lib/email.ts`, Resend REST API via native fetch) — best-effort side effect of `notifyUser`: after the in-app row is created, **important types only** are emailed (`timesheet.submitted/approved/rejected`, `expense.rejected`, `INVOICE_DUE_SOON`, `PROJECT_OVERRUN`, `LOW_MARGIN`). **Global kill-switch** `AppSetting.emailNotificationsEnabled` **defaults to disabled** — `maybeSendNotificationEmail` reads it via the cached `getAppSettings()` and early-returns, so no mail is sent until an admin enables it (Settings → "Email Notifications" card, MGMT/SUPER_ADMIN; dedicated `PUT /api/app-settings/email-notifications`, not in OpenAPI). In-app notifications are unaffected. Fired without awaiting, never throws, 5s timeout; skips deleted/invalid recipients; never logs the provider response body (PII — log `{status,domain,errorCode}` only). **No scheduler** — daily rules still only fire on MGMT dashboard load. From `notifications@mail.psa4pmo.xyz` (verified). Env levers (optional, sane defaults in code): `EMAIL_SEND_ALLOWLIST` (if set, only those exact addresses get mail — set to a test address before go-live to avoid bouncing seed/test emails, then clear), `EMAIL_SEND_BLOCKLIST_DOMAINS`, `EMAIL_FROM`, `APP_BASE_URL` (email links), `EMAIL_LOGO_URL`, `EMAIL_REPLY_TO`. Branded with the white-on-dark ITSEC wordmark (`artifacts/web/public/itsec-logo-white.png`, served at app root) + red CTA (`#e10e19`); the prod logo URL only resolves after a republish ships the asset.
...
- **Schema changes use Prisma Migrate** (not `db push`): edit `schema.prisma` → `pnpm --filter @workspace/db run migrate` (creates + applies a migration to dev, asks for a name) → commit the new folder under `lib/db/prisma/migrations/`. Merges into main apply pending migrations to the **dev** DB via `migrate:deploy` (`scripts/post-merge.sh`); **production is NOT auto-migrated** by the autoscale deploy. Prod baseline is `0_init` (already marked applied on dev + prod); legacy hand-written SQL is archived in `lib/db/prisma/manual-sql/` (reference only). Avoid `db push` now that migrations exist — it bypasses history and causes drift. For prod, run `DATABASE_URL=<prod> pnpm --filter @workspace/db run migrate:deploy` (no-op when nothing is pending).
