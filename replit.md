...
Intake: Sales create projects **only** via the Sales Pipeline lead-convert flow (`POST /api/leads/:id/convert`). Manual `POST /api/projects` is **hard-blocked for SALES** (403); `/projects/new` redirects Sales to `/leads` unless opened with `?leadId=` (the leads page links there per won/eligible lead). Via `?leadId=` the intake form requires a selected lead and a **required Resource Requirements** section — budget-only rows (role, headcount, mandays/person, daily rate; no person picker, no `ProjectResource`) computing `plannedMandays`/`estimatedCost`; the convert route enforces `plannedMandays > 0`. Server forces `status=DRAFT`, `salesId`, `pmId=null`. SALES stays in `writeRoles` (shared POST+PATCH) so Sales can edit own DRAFTs — the lock lives in the POST handler, not `writeRoles`. MGMT/PM keep manual create; PM assigns real people later in Resources; MGMT assigns PM (409 if already set). PM uses `DraftCompletionCard` → OBSERVATION.
...
- **Timeline** — drag-and-drop Gantt (`TaskGanttChart`/`GanttBar`), PATCH on pointer-up; `DependencyArrows` SVG. Server doesn't enforce dep timing — toast warns. **Critical Path** (`gantt/criticalPath.ts`, client-only CPM forward/backward pass over dated leaf tasks; topo sort + cycle guard; slack≤eps→critical; fully-isolated tasks — no preds AND no succs — are never marked) highlights the critical chain on the bars with a legend; interactions intact.
...
- **RAID** (`routes/raid.ts`) — `ProjectRaidItem`. Read = delivery team only (MGMT, PM, KONSULTAN, PRINCIPAL_* via `canViewRaid()`, mirrored FE/server), scoped per-project; write = MGMT/assigned-PM. CLOSED auto-stamps `closedAt`. **Risk scoring**: `riskScore = impact × likelihood` is **computed (never stored)** in the serializer/UI; `responseStrategy` (`RaidResponseStrategy` AVOID/MITIGATE/TRANSFER/ACCEPT, nullable) is editable. RaidTab shows a risk matrix + score; RBAC unchanged.
...
- **Change Requests** (`routes/change-requests.ts`, `tabs/ChangeRequestsTab.tsx`) — `ChangeRequest` (type SCOPE/SCHEDULE/COST; status DRAFT→APPROVED→APPLIED, or REJECTED). Tab + write gated MGMT/SUPER_ADMIN or owning-PM (`canManageChangeRequest`). Lifecycle: edit only while DRAFT (409 after APPROVED); **apply** atomically claims `updateMany({where:{id,status:"APPROVED"}})` inside `$transaction` (count===0→409, so concurrent double-apply yields exactly one APPLIED + one 409). Applying a SCHEDULE/COST CR writes the proposed fields onto the project **and** creates a new `ProjectBaseline` version (`source=CHANGE_REQUEST`, `changeRequestId` set); SCOPE CRs are audit-only. All transitions audited.
...
**Email** (`lib/email.ts`, Resend REST API via native fetch) — best-effort side effect of `notifyUser`: **important types only** are emailed (`timesheet.submitted/approved/rejected`, `expense.rejected`, `INVOICE_DUE_SOON`, `PROJECT_OVERRUN`, `LOW_MARGIN`). **Global kill-switch** `AppSetting.emailNotificationsEnabled` **defaults to disabled** — `maybeSendNotificationEmail` reads it via cached `getAppSettings()` and early-returns, so no mail sends until an admin enables it (Settings → "Email Notifications", MGMT/SUPER_ADMIN; `PUT /api/app-settings/email-notifications`, not in OpenAPI). In-app notifications are unaffected. Fired without awaiting, never throws, 5s timeout; skips deleted/invalid recipients; never logs the provider response body (PII — log `{status,domain,errorCode}` only). **No scheduler** — daily rules fire only on MGMT dashboard load. From `notifications@mail.psa4pmo.xyz` (verified). Optional env levers (sane defaults in code): `EMAIL_SEND_ALLOWLIST` (if set, only those exact addresses get mail — set to a test address before go-live to avoid bouncing seed emails, then clear), `EMAIL_SEND_BLOCKLIST_DOMAINS`, `EMAIL_FROM`, `APP_BASE_URL`, `EMAIL_LOGO_URL`, `EMAIL_REPLY_TO`. Branded with the white-on-dark ITSEC wordmark (`artifacts/web/public/itsec-logo-white.png`) + red CTA (`#e10e19`); the prod logo URL only resolves after a republish ships the asset.
...
- **EVM** (`computeEvm`, serializers.ts) — `BAC` (= current baseline `estimatedCost` if present, else project `estimatedCost`), `AC`=actualCost, `EV`=duration-weighted leaf-task progress × BAC (parents/summary excluded), `PV`=schedule-elapsed fraction × BAC using **baseline dates when a current baseline exists, else project start/end**. Derives `CPI/SPI/EAC/ETC/VAC/TCPI`, all null-safe (AC=0, PV=null, BAC≤0, no dated leaf tasks → `insufficientData` or a `reason` note). Surfaced in `GET /projects/:id/financials.evm`, gated by `canViewProjectFinancials`; FE `EvmPanel.tsx` (renders `reason` when schedule basis missing).
- **Baseline** (`ProjectBaseline`: `version`, `isCurrent`, `source` ACTIVATION/CHANGE_REQUEST/MANUAL, optional `changeRequestId`, snapshot of start/end/plannedMandays/estimatedCost/contractValue). One captured on the **ACTIVE** transition (`routes/projects.ts` PATCH). **Single-current invariant** held by `@@unique([projectId,version])` + create-and-flip-prior in one `$transaction` (no partial index — concurrent creates collide on version and roll back); serializer picks current via `findFirst orderBy version desc`. `financials.baseline` + `baselineVariance` (current vs baseline) drive the variance UI.
...
- **Schema changes use Prisma Migrate** (not `db push`): edit `schema.prisma` → `pnpm --filter @workspace/db run migrate` (creates + applies to dev, asks for a name) → commit the new folder under `lib/db/prisma/migrations/`. Merges into main auto-apply to the **dev** DB (`scripts/post-merge.sh`); **production is NOT auto-migrated** by the autoscale deploy — apply prod manually before/after republish: `bash scripts/release-prod-migrate.sh` (or `DATABASE_URL=<prod> pnpm --filter @workspace/db run migrate:deploy`). Full steps in `docs/RELEASE-CHECKLIST.md`. Prod baseline `0_init` (marked applied on dev+prod); legacy SQL archived in `lib/db/prisma/manual-sql/` (reference only). `db push` is guarded behind `ALLOW_DB_PUSH=1` — prefer migrations.
...
**Sample timesheet hours cap** (`lib/db/src/cap-daily-hours.ts`): demo generators could stack a user over impossible daily totals (broke Work Hours Compliance). `capUserDailyHours()` (final step of `seed.ts` + `sample-demo-enrichment.ts`) scales any (user, UTC day) over 8h down to 8h via a largest-remainder half-hour allocator. **`syntheticOnly` by default** — only touches generator-tagged rows (U+200B marker or `[sample]`), never real timesheets.
