...
Intake: Sales fills `/projects/new` (server forces `status=DRAFT`, `salesId`, `pmId=null`). A **required Resource Requirements** section uses budget-only rows (role, headcount, mandays/person, daily rate — no person picker, no `ProjectResource`) to compute `plannedMandays`/`estimatedCost`, sent on both create paths (`POST /api/projects`, `POST /api/leads/:id/convert`); server enforces `plannedMandays > 0` so the initial cost can't be bypassed. PM assigns real people later in Resources. MGMT assigns PM (409 if already set). PM uses `DraftCompletionCard` → OBSERVATION.
