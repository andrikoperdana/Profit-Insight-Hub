...
- **Timesheets** (`routes/timesheets.ts`) — all entries on the project, filterable + KPI cards. PM-of-project & MGMT Approve/Reject/Delete + "Approve All Submitted". Backed by `GET /api/timesheets?projectId=...`. Mutations invalidate timesheets/project/financials keys. **Approval notifications** (`lib/notifications.ts` `notifyUser`, persisted `Notification` rows surfaced in the Header bell via `GET /api/notifications`, polled 60s): submitting/resubmitting (single, bulk create, `/submit`) notifies the project's PM ("awaiting approval", link `/approvals`); approve (single + bulk) notifies the submitter ("approved"), reject notifies the submitter with the reason (link `/timesheets`). Actor==recipient is skipped (auto-approve, MGMT self). `/approve` and `/reject` are guarded to SUBMITTED-only (409 otherwise); `/submit` only notifies on a real transition into SUBMITTED — so repeated calls don't spam.
...
**Time tracking menu visibility** (`Sidebar.tsx`): the "Time Tracking" (`/timesheets`) link is hidden for Site Admin, Finance, HR, **and now Management + Sales** — these roles don't log their own hours. Sales also loses the "My Timesheet" personal view (kept My Tasks/My Expenses). Routes stay reachable (no `allowRoles` change) so PM/MGMT team-oversight isn't broken; this is a UI-menu change only. **Approval Inbox** (`/approvals`) sidebar link + the Header bell "Pending approvals" item are **PROJECT_MANAGER-only** — MGMT does not approve timesheets, so both are hidden for MGMT (route still reachable).
