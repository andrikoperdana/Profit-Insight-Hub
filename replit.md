...
- **RAID** — `ProjectRaidItem` (type RISK/ASSUMPTION/ISSUE/DEPENDENCY, impact LOW–CRITICAL, likelihood LOW/MEDIUM/HIGH, status OPEN/MITIGATING/CLOSED, owner, mitigation, dueDate). Endpoints in `routes/raid.ts`: `GET/POST /api/projects/:id/raid`, `PATCH/DELETE /api/raid/:itemId`. Read restricted to the project delivery team — MANAGEMENT, PROJECT_MANAGER, KONSULTAN, and the three PRINCIPAL_* roles (`canViewRaid()` in both `lib/roles.ts`, mirrored frontend/server; Sales/TW/Admin Project/Finance/HR/Site Admin get 403 and no RAID tab), still further scoped per-project by `userCanAccessProject`; write = MGMT or assigned PM. Status → CLOSED auto-stamps `closedAt`. UI tab `pages/projects/tabs/RaidTab.tsx` groups by type with 4 KPI cards.
