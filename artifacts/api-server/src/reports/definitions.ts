...
const profitabilityPerProject: ReportDefinition = {
  ...
  description: "Margin & profit per projek dengan filter status/PM/periode. Sortir margin terburuk lebih dulu.",
  ...
  filters: [
    ...
    { key: "startFrom", label: "Mulai dari", type: "date" },
    { key: "startTo", label: "Sampai", type: "date" },
  ],
  columns: [
  ...
  query: async (ctx) => {
    ...
    const rows: ReportRow[] = projects.map((p) => {
      ...
      return {
        ...
        pmName: (p as any).pm?.name ?? "Belum di-assign",
        status: p.status,
...
const marginTrendByBu: ReportDefinition = {
  ...
  description: "Trend margin & profit bulanan per Business Unit (group berdasarkan BU dari PM project).",
  category: "profitability",
  scope: ["MANAGEMENT"],
  filters: [
    { key: "year", label: "Tahun", type: "year", optionsSource: "yearList", defaultValue: String(new Date().getFullYear()) },
    { key: "businessUnitId", label: "Business Unit", type: "select", optionsSource: "businessUnits" },
  ],
  columns: [
    { key: "month", label: "Bulan", type: "month", width: 90 },
...
const profitabilityPerClient: ReportDefinition = {
  ...
  description: "Total revenue, cost, profit, dan margin per klien. Sortir profit terbesar lebih dulu.",
  category: "profitability",
  scope: ["MANAGEMENT"],
  filters: [
    { key: "status", label: "Status Project", type: "select", optionsSource: "projectStatuses" },
    { key: "startFrom", label: "Mulai dari", type: "date" },
    { key: "startTo", label: "Sampai", type: "date" },
  ],
  columns: [
...
const resourceUtilization: ReportDefinition = {
  ...
  description: "Utilisasi konsultan/TW dalam periode tertentu — planned vs actual mandays, jumlah project aktif.",
  category: "operations",
  scope: ["MANAGEMENT", "PROJECT_MANAGER"],
  filters: [
    { key: "from", label: "Periode dari", type: "date", defaultValue: new Date(new Date().getFullYear(), new Date().getMonth() - 1, 1).toISOString().slice(0, 10) },
    { key: "to", label: "Periode sampai", type: "date", defaultValue: new Date().toISOString().slice(0, 10) },
    { key: "businessUnitId", label: "Business Unit", type: "select", optionsSource: "businessUnits" },
  ...
  columns: [
    { key: "userName", label: "Nama", type: "string", width: 160 },
...
const projectBurnRate: ReportDefinition = {
  ...
  description: "Selisih planned vs actual mandays per projek aktif. Ranking by variance (over budget dulu).",
  category: "operations",
  scope: ["MANAGEMENT", "PROJECT_MANAGER"],
  filters: [
...
const pmWorkload: ReportDefinition = {
  ...
  description: "Beban kerja per PM — jumlah project per status, total nilai kontrak, avg margin.",
  category: "operations",
  scope: ["MANAGEMENT"],
  filters: [],
  columns: [
...
const billingAging: ReportDefinition = {
  ...
  description: "Daftar billing milestone dengan aging hari overdue. Sortir terlama belum dibayar lebih dulu.",
  category: "cashflow",
  scope: ["MANAGEMENT", "PROJECT_MANAGER"],
  filters: [
  ...
  columns: [
    ...
    { key: "vat", label: "PPN", type: "currency", align: "right", width: 110, total: "sum" },
...
const cashInflowForecast: ReportDefinition = {
  ...
  description: "Proyeksi penerimaan kas bulanan dari billing milestone (PLANNED + INVOICED).",
  category: "cashflow",
  scope: ["MANAGEMENT"],
  filters: [
    { key: "year", label: "Tahun", type: "year", optionsSource: "yearList", defaultValue: String(new Date().getFullYear()) },
  ...
  columns: [
    ...
    { key: "expectedVat", label: "Expected PPN", type: "currency", align: "right", width: 130, total: "sum" },
...
const expenseReport: ReportDefinition = {
  ...
  description: "Daftar expense lengkap dengan submitter, kategori, status approval, approver.",
  category: "cashflow",
  scope: ["MANAGEMENT", "PROJECT_MANAGER"],
  filters: [
    { key: "from", label: "Dari", type: "date" },
    { key: "to", label: "Sampai", type: "date" },
    { key: "category", label: "Kategori", type: "select", optionsSource: "expenseCategories" },
  ...
  columns: [
    { key: "spentAt", label: "Tanggal", type: "date", width: 100 },
    { key: "projectCode", label: "Project", type: "string", width: 90 },
    { key: "projectName", label: "Name", type: "string", width: 180 },
    { key: "category", label: "Kategori", type: "string", width: 100 },
    { key: "description", label: "Deskripsi", type: "string", width: 200 },
    { key: "submitter", label: "Submitter", type: "string", width: 130 },
    { key: "amount", label: "Jumlah", type: "currency", align: "right", width: 130, total: "sum" },
...
const ppnDetail: ReportDefinition = {
  id: "ppn-detail",
  name: "PPN Detail per Invoice",
  description: "Rincian DPP & PPN 11% per invoice untuk SPT Masa PPN. Filter tahun/bulan/status.",
  category: "compliance",
  scope: ["MANAGEMENT"],
  filters: [
    { key: "year", label: "Tahun", type: "year", optionsSource: "yearList", defaultValue: String(new Date().getFullYear()) },
    { key: "month", label: "Bulan", type: "select", options: Array.from({ length: 12 }, (_, i) => ({ value: String(i + 1), label: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][i]! })) },
    { key: "status", label: "Status", type: "select", options: [
      { value: "INVOICED", label: "Invoiced (belum bayar)" },
      { value: "PAID", label: "Paid" },
    ] },
    { key: "clientId", label: "Client", type: "select", optionsSource: "clients" },
  ],
  columns: [
    { key: "invoicedAt", label: "Tgl Invoice", type: "date", width: 100 },
    ...
    { key: "milestoneName", label: "Milestone", type: "string", width: 160 },
    { key: "dpp", label: "DPP", type: "currency", align: "right", width: 130, total: "sum" },
    { key: "vat", label: "PPN 11%", type: "currency", align: "right", width: 110, total: "sum" },
    { key: "total", label: "Total", type: "currency", align: "right", width: 130, total: "sum" },
    { key: "status", label: "Status", type: "badge", width: 100, badgeMap: billingStatusBadgeMap },
