...
const MAX_W = 600; // px at 96dpi ≈ 6.25in usable width
const MAX_H = 780; // keep tall full-page captures on one page

function img(file, { fromDiagrams = false, maxW = MAX_W, maxH = MAX_H } = {}) {
...
function cell(content, widthPct, { bold = false, fill } = {}) {
        ...
        default: new Header({
          children: [
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: "SecureProfit Hub — Board Overview · Confidential",
                  size: 16,
                  color: "94A3B8",
                }),
              ],
